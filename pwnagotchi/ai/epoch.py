...
from ai_inference import HandshakeAnalyzer
import pwnagotchi
...
class Epoch(object):
    def __init__(self, config, use_multicore=True):
        self.epoch = 0
        self.config = config
        self.use_multicore = use_multicore
        self.executor = ThreadPoolExecutor(max_workers=2) if use_multicore else None
    ...
    def wait_for_epoch_data(self, with_observation=True, timeout=None):
        ...
        return self._epoch_data if not with_observation else {**self._observation, **self._epoch_data}
    ...
    def next(self):
        ...
        logging.info(
            ...
            "avg_bond=%.2f hops=%d missed=%d deauths=%d assocs=%d handshakes=%d cpu=%d%% mem=%d%% temperature=%dC reward=%s" %
            (self.epoch,
